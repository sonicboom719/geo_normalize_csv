...
def fix_duplicate_town_names(addr):
    """重複する町名を修正する関数"""
    # よくある重複パターンを修正
    patterns = [
        (r'美里町美里町', '美里町'),
...
def get_best_latlng(address, api_key, gsi_check=True, distance_threshold=200, priority="gsi"):
    lat1, lon1 = get_gmap_latlng(address, api_key)  # Google Maps APIの座標
    lat2, lon2 = get_gsi_latlng(address)            # 国土地理院APIの座標
    
    # 両方の座標を取得できた場合の距離計算
    distance = None
    if lat1 is not None and lat2 is not None:
        distance = haversine(lat1, lon1, lat2, lon2)
        print(f"'{address}' の座標差: Google座標({lat1:.6f}, {lon1:.6f}) vs 国土地理院座標({lat2:.6f}, {lon2:.6f}) = {int(distance)}m")
    ...
    if lat1 is not None and lat2 is None:
        print(f"'{address}' の座標: Google座標({lat1:.6f}, {lon1:.6f})のみ取得")
        return lat1, lon1, None, None, None, "google"
    if lat2 is not None and lat1 is None:
        print(f"'{address}' の座標: 国土地理院座標({lat2:.6f}, {lon2:.6f})のみ取得")
        return None, None, lat2, lon2, None, "gsi"

    # 両方取得できた場合の処理（距離チェックは行わず、両方の座標を返す）
    return lat1, lon1, lat2, lon2, distance, "both"
...
def render_template(template_str, row, cache, full_api_address, api_key, sleep_msec, gsi_check, gsi_dist, priority):
    def replacer(match):
        ...
        elif token in ("lat", "long", "gsi_lat", "gsi_long", "distance"):
            if "latlng" not in cache:
                gmap_lat, gmap_lng, gsi_lat, gsi_lng, distance, source = get_best_latlng(full_api_address, api_key, gsi_check, gsi_dist, priority)
                cache["latlng"] = (gmap_lat, gmap_lng, gsi_lat, gsi_lng, distance)
                cache["source"] = source
                time.sleep(sleep_msec / 1000)
            
            gmap_lat, gmap_lng, gsi_lat, gsi_lng, distance = cache["latlng"]
            
            if token == "lat":
                return str(clean(gmap_lat)) if gmap_lat is not None else ""
            elif token == "long":
                return str(clean(gmap_lng)) if gmap_lng is not None else ""
            elif token == "gsi_lat":
                return str(clean(gsi_lat)) if gsi_lat is not None else ""
            elif token == "gsi_long":
                return str(clean(gsi_lng)) if gsi_lng is not None else ""
            elif token == "distance":
                return str(clean(distance)) if distance is not None else ""
            else:
                return ""
        else:
            return ""
...
def process(config_path):
    ...
    for idx, row in enumerate(input_rows, start=1):
        ...
        normalized_address = fix_duplicate_town_names(normalized_address)
